...
interface MediaMessageProps {
  mediaUrl:  string;
  mediaType: string;
  fileName?: string; // nombre original del documento (si se conoce)
  content?:  string; // caption opcional
  isSticker?: boolean;
}

export function MediaMessage({ mediaUrl, mediaType, fileName, content, isSticker = false }: MediaMessageProps) {
  if (isSticker) {
    return (
      <a
        href={mediaUrl}
    ...
    return (
      <div className="flex flex-col gap-1.5">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2.5 bg-zinc-900/60 rounded-xl px-3 py-2.5 hover:bg-zinc-900 transition-colors group max-w-[260px]"
        >
          <span className="text-2xl shrink-0">📄</span>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-zinc-100 group-hover:text-violet-300 transition-colors truncate">
              {fileName || 'Documento PDF'}
            </p>
            <p className="text-[10px] text-zinc-500">Toca para abrir</p>
          </div>
          <span className="ml-auto shrink-0 text-zinc-500 group-hover:text-zinc-300">↗</span>
        </a>
        {content && <p className="text-sm leading-relaxed whitespace-pre-wrap">{content}</p>}
      </div>
    );
